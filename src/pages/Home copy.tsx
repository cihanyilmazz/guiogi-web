import React from "react";
import { Card, Row, Col, Layout, Tag, Button } from "antd";
const { Content } = Layout;
import {
  EnvironmentOutlined,
  CalendarOutlined,
  UserOutlined,
  StarFilled,
  SafetyOutlined,
  HeartOutlined,
  CheckCircleOutlined,
  ClockCircleOutlined,
} from "@ant-design/icons";
import HeroBanner from "../components/banner";

const { Meta } = Card;

const App: React.FC = () => {
  const tourData = [
    {
      id: 1,
      title: "Kapadokya Balon Turu",
      description:
        "Eşsiz peri bacaları manzarası eşliğinde unutulmaz bir balon deneyimi. Gün doğumu manzarasında fotoğraf çekimi ve geleneksel kahvaltı dahil.",
      coverImage:
        "https://images.unsplash.com/photo-1541414778236-3002b7bd0155?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Nevşehir, Türkiye",
      duration: "2 Gece 3 Gün",
      groupSize: "12 Kişi",
      category: "Doğa",
      season: "Yaz",
      guide: "Ahmet Yılmaz",
      rating: 4.8,
      reviewCount: 124,
      specialOffer: "Erken Rezervasyon Fırsatı",
      included: [
        "Balon uçuşu",
        "Geleneksel kahvaltı",
        "Profesyonel fotoğraf",
        "Sigorta",
      ],
      highlights: ["Gün doğumu manzarası", "Peri bacaları", "Açık hava müzesi"],
    },
    {
      id: 2,
      title: "Pamukkale Termal Tur",
      description:
        "Antik havuzlar ve travertenlerde benzersiz bir termal deneyim. Kleopatra'nın yüzdüğü havuzda yüzme şansı.",
      coverImage:
        "https://images.unsplash.com/photo-1593238739363-c8ab33238c56?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Denizli, Türkiye",
      duration: "1 Gece 2 Gün",
      groupSize: "8 Kişi",
      category: "Termal",
      season: "Tüm Yıl",
      guide: "Mehmet Demir",
      rating: 4.6,
      reviewCount: 89,
      specialOffer: "Aile Paketi",
      included: ["Termal havuz girişi", "Konaklama", "Rehberlik", "Transfer"],
      highlights: ["Travertenler", "Antik havuz", "Hierapolis antik kenti"],
    },
    {
      id: 3,
      title: "Ege Mavi Yolculuk",
      description:
        "Mavi bayraklı koylarda lüks yat ile unutulmaz bir mavi yolculuk. Özel koylarda yüzme molaları ve deniz ürünleri.",
      coverImage:
        "https://images.unsplash.com/photo-1593238640783-3e1e9d72f9a3?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Muğla, Türkiye",
      duration: "4 Gece 5 Gün",
      groupSize: "15 Kişi",
      category: "Deniz",
      season: "Yaz",
      guide: "Ayşe Kaya",
      rating: 4.9,
      reviewCount: 156,
      specialOffer: "Haftalık Kiralama",
      included: [
        "Yat konaklama",
        "Tam pansiyon",
        "Tekne turu",
        "Sualtı ekipmanı",
      ],
      highlights: ["Özel koylar", "Güneş batımı", "Deniz sporları"],
    },
    {
      id: 4,
      title: "Karadeniz Yayla Turu",
      description:
        "Yeşilin her tonunu görebileceğiniz yaylalar ve şelaleler turu. Yöresel yemekler ve doğa yürüyüşleri.",
      coverImage:
        "https://images.unsplash.com/photo-1501785888041-af3ef285b470?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Rize, Türkiye",
      duration: "3 Gece 4 Gün",
      groupSize: "10 Kişi",
      category: "Doğa",
      season: "İlkbahar-Yaz",
      guide: "Fatma Şahin",
      rating: 4.7,
      reviewCount: 67,
      specialOffer: "Fotoğrafçılık Turu",
      included: [
        "Doğa yürüyüşü",
        "Yöresel konaklama",
        "Rehber",
        "Fotoğraf rehberi",
      ],
      highlights: ["Yaylalar", "Şelaleler", "Yöresel mutfak"],
    },
    {
      id: 5,
      title: "İstanbul Kültür Turu",
      description:
        "Tarihi yarımada ve boğaz turu ile İstanbul'u keşfedin. Topkapı Sarayı, Ayasofya ve Boğaz turu.",
      coverImage:
        "https://images.unsplash.com/photo-1524231757912-21f4fe3a7200?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "İstanbul, Türkiye",
      duration: "3 Gece 4 Gün",
      groupSize: "20 Kişi",
      category: "Kültür",
      season: "Tüm Yıl",
      guide: "Ali Öztürk",
      rating: 4.5,
      reviewCount: 203,
      specialOffer: "Öğrenci İndirimi",
      included: [
        "Müze girişleri",
        "Boğaz turu",
        "Ulaşım",
        "Profesyonel rehber",
      ],
      highlights: ["Tarihi yarımada", "Boğaz turu", "Müzeler"],
    },
    {
      id: 6,
      title: "Antalya Dalış Turu",
      description:
        "Akdeniz'in berrak sularında profesyonel rehberlikte dalış keyfi. Sualtı yaşamını keşfedin.",
      coverImage:
        "https://images.unsplash.com/photo-1506929562872-bb421503ef21?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Antalya, Türkiye",
      duration: "2 Gece 3 Gün",
      groupSize: "6 Kişi",
      category: "Spor",
      season: "Yaz",
      guide: "Can Aydın",
      rating: 4.8,
      reviewCount: 78,
      specialOffer: "İlk Dalış Paketi",
      included: ["Dalış ekipmanı", "Eğitmen", "Tekne turu", "Sertifika"],
      highlights: ["Sualtı yaşamı", "Batık keşfi", "Dalış eğitimi"],
    },
    {
      id: 7,
      title: "Doğu Anadolu Ekspresi",
      description:
        "Doğu'nun gizemli şehirlerini ve doğal güzelliklerini keşfedin. Kültürlerin kesişme noktası.",
      coverImage:
        "https://images.unsplash.com/photo-1469474968028-56623f02e42e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Erzurum, Türkiye",
      duration: "5 Gece 6 Gün",
      groupSize: "14 Kişi",
      category: "Kültür",
      season: "Yaz",
      guide: "Zeynep Yıldız",
      rating: 4.4,
      reviewCount: 45,
      specialOffer: "Kültür Pasaportu",
      included: [
        "Tarihi mekanlar",
        "Yöresel konaklama",
        "Kültür rehberi",
        "El sanatı atölyesi",
      ],
      highlights: [
        "Tarihi kervansaraylar",
        "Dağ manzaraları",
        "Kültürel etkileşim",
      ],
    },
    {
      id: 8,
      title: "Marmaris Yat Turu",
      description:
        "Lüks yatlarla mavi yolculuk ve özel koylarda yüzme molaları. Akdeniz'in incisinde tatil.",
      coverImage:
        "https://images.unsplash.com/photo-1570077188670-e3a8d69ac5ff?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80",
      location: "Muğla, Türkiye",
      duration: "3 Gece 4 Gün",
      groupSize: "12 Kişi",
      category: "Deniz",
      season: "Yaz",
      guide: "Burak Koç",
      rating: 4.9,
      reviewCount: 112,
      specialOffer: "Honeymoon Paketi",
      included: [
        "Lüks yat",
        "Gurme yemekler",
        "Spa hizmetleri",
        "Özel koy turu",
      ],
      highlights: ["Lüks konfor", "Özel koylar", "Gurme deneyim"],
    },
  ];

  const features = [
    {
      icon: <SafetyOutlined />,
      title: "Güvenli Seyahat",
      description: "Tüm turlarımız seyahat sigortası ile güvence altında",
    },
    {
      icon: <ClockCircleOutlined />,
      title: "7/24 Destek",
      description: "Yol boyunca size destek sağlayan ekibimiz",
    },
    {
      icon: <CheckCircleOutlined />,
      title: "Kalite Garantisi",
      description: "Yüksek standartlarda hizmet kalitesi",
    },
    {
      icon: <UserOutlined />,
      title: "Uzman Rehberler",
      description: "Alanında uzman, sertifikalı rehber kadromuz",
    },
  ];

  const whyChooseUs = [
    "20+ yıllık seyahat deneyimi",
    "5000+ mutlu müşteri",
    "50+ farklı tur rotası",
    "%98 müşteri memnuniyeti",
    "Eko-turizm ilkeleri",
    "Kişiye özel tur planlama",
  ];

  return (
    <Layout className="min-h-screen bg-white">
      <Content className="h-full">
        {/* Hero Banner */}
        <HeroBanner />

        {/* Welcome Section */}
        <div className="bg-white py-12 border-b border-gray-100">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-4">
                Türkiye'nin En Özel Turlarını Keşfedin
              </h2>
              <p className="text-gray-600 text-lg mb-6">
                Guiaogi Turizm olarak, 20 yılı aşkın deneyimimizle Türkiye'nin
                dört bir yanındaki eşsiz güzellikleri keşfetmeniz için özenle
                hazırlanmış tur paketleri sunuyoruz. Her bütçeye ve her zevke
                uygun seçeneklerimizle unutulmaz anılar biriktirmenize yardımcı
                oluyoruz.
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                {whyChooseUs.map((item, index) => (
                  <Tag
                    key={index}
                    className="bg-gray-50 text-gray-700 border-gray-200 text-sm py-1 px-3"
                  >
                    ✓ {item}
                  </Tag>
                ))}
              </div>
            </div>
          </div>
        </div>

        {/* Features Section */}
        <div className="bg-white py-12">
          <div className="container mx-auto px-4">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">
                Neden Guiaogi Turizm?
              </h2>
              <p className="text-gray-600">
                Size özel sunduğumuz avantajlar ve garantiler
              </p>
            </div>

            <Row gutter={[24, 24]}>
              {features.map((feature, index) => (
                <Col xs={24} sm={12} md={6} key={index}>
                  <div className="text-center p-6 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-300 border border-gray-200 h-full">
                    <div className="text-3xl text-blue-600 mb-4 flex justify-center">
                      {feature.icon}
                    </div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">
                      {feature.title}
                    </h3>
                    <p className="text-gray-600 text-sm">
                      {feature.description}
                    </p>
                  </div>
                </Col>
              ))}
            </Row>
          </div>
        </div>

        {/* Tours Section */}
        <div className="bg-gray-50 py-12">
          <div className="container mx-auto px-4">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">
                Öne Çıkan Turlarımız
              </h2>
              <p className="text-gray-600 mb-4">
                En çok tercih edilen ve en yüksek puan alan tur paketlerimiz
              </p>
            </div>

            <Row gutter={[16, 16]}>
              {tourData.map((tour) => (
                <Col xs={24} sm={12} md={12} lg={8} xl={6} key={tour.id}>
                  <Card
                    className="tour-card w-full hover:shadow-lg transition-all duration-300 border border-gray-200 rounded-lg bg-white"
                    cover={
                      <div className="relative">
                        <img
                          alt={tour.title}
                          src={tour.coverImage}
                          className="h-48 w-full object-cover rounded-t-lg"
                          onError={(e) => {
                            e.currentTarget.src =
                              "https://images.unsplash.com/photo-1488646953014-85cb44e25828?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=400&h=250&q=80";
                          }}
                        />
                        <div className="absolute top-3 left-3">
                          <Tag color="blue" className="text-xs font-medium">
                            {tour.category}
                          </Tag>
                        </div>
                        <div className="absolute top-3 right-3">
                          <Tag color="green" className="text-xs font-medium">
                            {tour.specialOffer}
                          </Tag>
                        </div>
                        <div className="absolute bottom-3 left-3">
                          <div className="flex items-center bg-black/70 text-white px-2 py-1 rounded text-xs">
                            <StarFilled className="text-yellow-400 mr-1" />
                            <span>{tour.rating}</span>
                            <span className="ml-1">({tour.reviewCount})</span>
                          </div>
                        </div>
                      </div>
                    }
                    actions={[
                      <div key="info" className="text-center px-2">
                        <Button
                          type="primary"
                          block
                          className="bg-blue-600 hover:bg-blue-700 font-medium"
                        >
                          Detaylı Bilgi ve Teklif Al
                        </Button>
                      </div>,
                    ]}
                  >
                    <Meta
                      title={
                        <div className="flex justify-between items-start">
                          <span className="text-base font-semibold text-gray-800">
                            {tour.title}
                          </span>
                        </div>
                      }
                      description={
                        <div className="space-y-3 mt-2">
                          <p className="text-gray-600 text-sm leading-relaxed">
                            {tour.description}
                          </p>

                          <div className="space-y-2">
                            <div className="flex items-center text-xs text-gray-500">
                              <EnvironmentOutlined className="mr-2 text-blue-500" />
                              <span className="truncate font-medium">
                                {tour.location}
                              </span>
                            </div>

                            <div className="flex items-center text-xs text-gray-500">
                              <CalendarOutlined className="mr-2 text-green-500" />
                              <span>{tour.duration}</span>
                              <span className="mx-2">•</span>
                              <UserOutlined className="mr-1 text-purple-500" />
                              <span>{tour.groupSize}</span>
                            </div>

                            <div className="flex items-center text-xs text-gray-500">
                              <UserOutlined className="mr-2 text-orange-500" />
                              <span className="truncate">
                                Profesyonel Rehber: {tour.guide}
                              </span>
                            </div>
                          </div>

                          {/* Tur Özellikleri */}
                          <div className="pt-3 border-t border-gray-100">
                            <p className="text-xs font-medium text-gray-700 mb-2">
                              Tur Öne Çıkanları:
                            </p>
                            <div className="flex flex-wrap gap-1">
                              {tour.highlights.map((highlight, index) => (
                                <Tag
                                  key={index}
                                  className="bg-blue-50 text-blue-700 border-blue-200 text-xs m-0"
                                >
                                  {highlight}
                                </Tag>
                              ))}
                            </div>
                          </div>

                          {/* Dahil Olanlar */}
                          <div className="pt-3 border-t border-gray-100">
                            <p className="text-xs font-medium text-gray-700 mb-2">
                              Pakete Dahil Olanlar:
                            </p>
                            <div className="flex flex-wrap gap-1">
                              {tour.included.map((item, index) => (
                                <Tag
                                  key={index}
                                  className="bg-green-50 text-green-700 border-green-200 text-xs m-0"
                                >
                                  ✓ {item}
                                </Tag>
                              ))}
                            </div>
                          </div>

                          {/* Mevsim Bilgisi */}
                          <div className="flex items-center justify-between pt-3">
                            <Tag color="orange" className="text-xs">
                              {tour.season} Mevsimi
                            </Tag>
                            <span className="text-xs text-gray-500">
                              ⭐ {tour.rating} Puan
                            </span>
                          </div>
                        </div>
                      }
                    />
                  </Card>
                </Col>
              ))}
            </Row>
          </div>
        </div>

        {/* Stats Banner */}
        <div className="bg-white py-16">
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-3">
                Güveninizin Göstergesi
              </h2>
              <div className="w-24 h-1 bg-blue-500 mx-auto mb-6"></div>
              <p className="text-gray-600 max-w-2xl mx-auto">
                20 yılı aşkın deneyimimizin rakamlarla ifadesi
              </p>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 max-w-4xl mx-auto">
              <div className="text-center">
                <div className="text-4xl md:text-5xl font-bold text-blue-600 mb-2">
                  20+
                </div>
                <div className="text-gray-600 text-sm md:text-base">
                  Yıllık Deneyim
                </div>
              </div>

              <div className="text-center">
                <div className="text-4xl md:text-5xl font-bold text-blue-600 mb-2">
                  5000+
                </div>
                <div className="text-gray-600 text-sm md:text-base">
                  Mutlu Müşteri
                </div>
              </div>

              <div className="text-center">
                <div className="text-4xl md:text-5xl font-bold text-blue-600 mb-2">
                  50+
                </div>
                <div className="text-gray-600 text-sm md:text-base">
                  Tur Rotası
                </div>
              </div>

              <div className="text-center">
                <div className="text-4xl md:text-5xl font-bold text-blue-600 mb-2">
                  %98
                </div>
                <div className="text-gray-600 text-sm md:text-base">
                  Memnuniyet Oranı
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Info Banner */}
        <div className="relative py-16 md:py-20">
          <div
            className="absolute inset-0 bg-cover bg-center bg-no-repeat"
            style={{
              backgroundImage:
                "url(https://images.unsplash.com/photo-1519681393784-d120267933ba?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80)",
            }}
          />
          <div className="absolute inset-0 bg-gradient-to-r from-gray-900/70 to-gray-800/70" />

          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-3xl mx-auto text-center text-white">
              <h2 className="text-2xl md:text-4xl font-bold mb-6">
                Hayalinizdeki Tatil Sadece Bir Tık Uzağınızda
              </h2>

              <p className="text-gray-200 text-lg leading-relaxed mb-8">
                Türkiye'nin en güzel rotalarında, uzman rehberler eşliğinde
                benzersiz deneyimler yaşayın. Size özel hazırlanmış tur
                paketlerimizle unutulmaz anılar biriktirin.
              </p>

              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button
                  type="primary"
                  size="large"
                  className="bg-white text-gray-900 hover:bg-gray-100 border-white hover:border-gray-100 h-12 px-8 font-semibold"
                >
                  📞 HEMEN ARAYIN
                </Button>
                <Button
                  size="large"
                  className="bg-transparent text-white hover:bg-white/20 border-2 border-white h-12 px-8 font-semibold"
                >
                  ✉️ TEKLİF İSTE
                </Button>
              </div>
            </div>
          </div>
        </div>

        {/* How It Works */}
        <div className="bg-white py-12">
          <div className="container mx-auto px-4">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">
                Nasıl Çalışıyoruz?
              </h2>
              <p className="text-gray-600">
                Tur rezervasyon sürecimiz sadece 3 kolay adımdan oluşuyor
              </p>
            </div>

            <Row gutter={[24, 24]}>
              <Col xs={24} md={8}>
                <div className="text-center p-6">
                  <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <span className="text-2xl font-bold text-gray-700">1</span>
                  </div>
                  <h3 className="text-lg font-semibold mb-3">Tur Seçimi</h3>
                  <p className="text-gray-600">
                    Size en uygun tur paketini seçin veya özel tur talebinizi
                    iletin
                  </p>
                </div>
              </Col>
              <Col xs={24} md={8}>
                <div className="text-center p-6">
                  <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <span className="text-2xl font-bold text-gray-700">2</span>
                  </div>
                  <h3 className="text-lg font-semibold mb-3">Rezervasyon</h3>
                  <p className="text-gray-600">
                    Online formu doldurarak veya telefonla kolayca rezervasyon
                    yapın
                  </p>
                </div>
              </Col>
              <Col xs={24} md={8}>
                <div className="text-center p-6">
                  <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <span className="text-2xl font-bold text-gray-700">3</span>
                  </div>
                  <h3 className="text-lg font-semibold mb-3">
                    Keyfini Çıkarın
                  </h3>
                  <p className="text-gray-600">
                    Tüm detaylar bizde, siz sadece unutulmaz tatilinizin tadını
                    çıkarın
                  </p>
                </div>
              </Col>
            </Row>
          </div>
        </div>

        {/* CTA Section */}
        <div className="bg-gray-50 py-12 border-t border-gray-200">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              <h3 className="text-2xl font-bold text-gray-800 mb-4">
                Size Özel Tur Paketleri İçin
              </h3>
              <p className="text-gray-600 mb-6">
                İhtiyaçlarınıza uygun tur paketleri, özel fiyat teklifleri ve
                detaylı bilgi için bizimle iletişime geçin. Uzman ekibimiz size
                en uygun seçenekleri sunmak için hazır.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Button
                  type="primary"
                  size="large"
                  className="bg-blue-600 hover:bg-blue-700 h-12 px-8 font-semibold"
                >
                  📞 0 (212) 123 45 67
                </Button>
                <Button
                  size="large"
                  className="border-blue-600 text-blue-600 hover:border-blue-700 hover:text-blue-700 h-12 px-8 font-semibold bg-white"
                >
                  ✉️ info@guiaogi.com
                </Button> 
              </div>
            </div>
          </div>
        </div>
      </Content>
    </Layout>
  );
};

export default App;
