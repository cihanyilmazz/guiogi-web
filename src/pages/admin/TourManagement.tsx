// pages/admin/TourManagement.tsx
...
const TourManagement: React.FC = () => {
  ...
  const fetchTours = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3005/tours');
      if (response.ok) {
  ...
  const handleDelete = async (tourId: string | number) => {
    try {
      const response = await fetch(`http://localhost:3005/tours/${tourId}`, {
        method: 'DELETE',
      });
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (editingTour) {
        // Güncelleme
        const response = await fetch(`http://localhost:3005/tours/${editingTour.id}`, {
          method: 'PUT',
          headers: {
