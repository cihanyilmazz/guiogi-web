...
const LanguageManagement: React.FC = () => {
  ...
  const saveTranslations = useCallback(async (langCode: string, translationsData: any) => {
    try {
      ...
      try {
        await fetch(`http://localhost:3005/translations/${langCode}`, {
          method: 'PUT',
          headers: {
  ...
  const handleDeleteLanguage = useCallback(async (code: string) => {
    try {
      const response = await fetch(`http://localhost:3005/languages/${code}`, {
        method: 'DELETE',
      });
  ...
  const handleSaveLanguage = useCallback(async (values: any) => {
    try {
      ...
      if (editingLanguage) {
        const response = await fetch(`http://localhost:3005/languages/${editingLanguage.code}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
