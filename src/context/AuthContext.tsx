...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:37',message:'Fetching users from json-server',data:{url:'http://localhost:3005/users?email='+encodeURIComponent(email)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      ...
      try {
        ...
        response = await fetch(`http://localhost:3005/api/users?email=${encodeURIComponent(email)}`, {
          method: 'GET',
          headers: {
        ...
        if (response.status === 404) {
          // #region agent log
          fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:55',message:'404 error - trying alternative endpoint',data:{alternativeUrl:'http://localhost:3005/users?email='+encodeURIComponent(email)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
          // #endregion
          
          response = await fetch(`http://localhost:3005/users?email=${encodeURIComponent(email)}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const register = async (userData: any) => {
    ...
    try {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:157',message:'Checking if email already exists',data:{email:userData.email,checkUrl:'http://localhost:3005/api/users?email='+encodeURIComponent(userData.email)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion

      let emailCheckResponse;
      try {
        // Önce /api/users endpoint'ini dene
        emailCheckResponse = await fetch(`http://localhost:3005/api/users?email=${encodeURIComponent(userData.email)}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (emailCheckResponse.status === 404) {
          // #region agent log
          fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:174',message:'404 error - trying alternative endpoint for email check',data:{alternativeUrl:'http://localhost:3005/users?email='+encodeURIComponent(userData.email)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
          // #endregion
          
          emailCheckResponse = await fetch(`http://localhost:3005/users?email=${encodeURIComponent(userData.email)}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:68',message:'Preparing fetch request',data:{url:'http://localhost:3005/api/users',userData:newUser},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      ...
      try {
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:75',message:'Fetch request starting',data:{url:'http://localhost:3005/api/users'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
        // #endregion
        
        response = await fetch('http://localhost:3005/api/users', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          // #region agent log
          fetch('http://127.0.0.1:7242/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'AuthContext.tsx:109',message:'404 error - trying alternative endpoint',data:{originalUrl:'http://localhost:3005/api/users',alternativeUrl:'http://localhost:3005/users'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
          // #endregion
          
          // /api/users 404 verdi, /users deneyelim
          const altResponse = await fetch('http://localhost:3005/users', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
