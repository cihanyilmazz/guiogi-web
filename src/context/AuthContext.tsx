...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (!foundUser) {
        // #region agent log
        fetch('http://49.13.94.27:3005/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'AuthContext.tsx:97', message: 'User not found', data: { email }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'B' }) }).catch(() => { });
        // #endregion
      ...
      if (foundUser.password !== password) {
        // #region agent log
        fetch('http://49.13.94.27:3005/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'AuthContext.tsx:104', message: 'Password mismatch', data: { email, passwordMatch: false }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'B' }) }).catch(() => { });
        // #endregion
        throw new Error('E-posta veya şifre hatalı');
      }
      ...
      // #region agent log
      fetch('http://49.13.94.27:3005/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'AuthContext.tsx:125', message: 'User state updated', data: { userSet: true }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
      // #endregion
    } catch (error: any) {
      // #region agent log
  ...
  const register = async (userData: any) => {
    ...
    try {
      ...
      // #region agent log
      fetch('http://49.13.94.27:3005/ingest/a29d79da-c2fb-4547-a375-e0d59332ce77', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'AuthContext.tsx:166', message: 'User state updated', data: { userSet: true }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
      // #endregion
    } catch (error: any) {
      // #region agent log
