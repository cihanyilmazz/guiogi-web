...
class TourService {
  async getAllTours(): Promise<Tour[]> {
    try {
      ...
      try {
        const dbResponse = await fetch('http://guiaogi.com/tours');
        if (dbResponse.ok) {
      ...
      try {
        const response = await fetch('http://guiaogi.com/api/tours');
        if (response.ok) {
  ...
  async getTourById(id: number | string): Promise<Tour> {
    try {
      ...
      try {
        const response = await fetch(`http://guiaogi.com/tours/${id}`);
        if (response.ok) {
